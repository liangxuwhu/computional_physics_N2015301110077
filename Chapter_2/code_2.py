...
class cannon_shell_trajectory:
    ...
    def run(self):
        while(self.y[-1] >= 0):
            self.v_x = self.v_x-self.B*self.p*self.v*self.v_x*math.exp(-self.y[-1]/self.y_0)*self.dt
            self.x.append(self.x[-1] + self.v_x * self.dt)
            self.v_y = self.v_y-self.g * self.dt-self.B*self.p*self.v*self.v_y*math.exp(-self.y[-1]/self.y_0)*self.dt
